...
export function validateZorm(text: string): ValidationResult | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inConfig) {
      ...
      if (line.startsWith("backend ")) {
        ...
        if (!backend) return { error: ValidatorError.InvalidConfig, line: i };
    ...
    if (line.startsWith("model ")) {
      ...
      modelName = line
        ...
        .trim()
        .replace(/[ {\t]/g, "");
      if (!modelName) return { error: ValidatorError.InvalidModel, line: i };
    ...
    if (line.startsWith("}")) {
      if (!modelOpen) return { error: ValidatorError.InvalidModel, line: i };
      modelOpen = false;
      continue;
    }
  ...
  if (!backendFound) return { error: ValidatorError.InvalidConfig, line: 0 };
  if (modelOpen)
