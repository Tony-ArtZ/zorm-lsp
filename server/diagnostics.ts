...
export function getDiagnosticsFromParseError(
  error: string,
  lines: string[],
  errorLine?: number
): Diagnostic[] {
  ...
  if (errorLine !== undefined) {
    ...
    if (error.includes("InvalidConfig")) {
      ...
      line = lines.findIndex(
        (l) => l.trim().startsWith("[config]") || l.includes("backend")
      );
      if (line === -1) line = 0;
      ...
      if (line === -1) line = lines.findIndex((l) => l.trim() === "}");
      if (line === -1) line = 0;
  ...
  diagnostics.push({
    ...
    range: Range.create(
      ...
      Position.create(line, lines[line]?.length || 1)
    ),
    message,
    source: "zorm-parser",
  });
